...
async def get_fresh_douyin_cookies(force: bool = False) -> str:
    ...
    # 1. Tenta Playwright se disponível
    try:
        from playwright.async_api import async_playwright
        log.info("🌐 [AutoCookie] Gerando cookies autênticos do Douyin via Playwright...")
        ...
        log.warning(f"⚠️ [AutoCookie] Playwright indisponível ({e}). Tentando fallback via HTTP direto...")
        try:
            headers = {
            ...
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                ...
                if cookies:
                    lines = ["# Netscape HTTP Cookie File", "# Auto-generated by HTTP Scrapper Fallback"]
                    for name, value in cookies.items():
                        lines.append(f".douyin.com\tTRUE\t/\tFALSE\t2147483647\t{name}\t{value}")
                    with open(COOKIE_CACHE_FILE, "w", encoding="utf-8") as f:
                        f.write("\n".join(lines) + "\n")
                    log.info(f"✅ [AutoCookie HTTP] {len(cookies)} cookies obtidos via HTTP!")
                    return COOKIE_CACHE_FILE
        except Exception as e_http:
            log.error(f"❌ [AutoCookie HTTP] Erro no fallback HTTP: {e_http}")
        return ""
...
async def refresh_and_propagate_douyin_cookies(force: bool = False) -> str:
    ...
    if not cookie_str:
        log.warning("Nenhum cookie do Douyin encontrado ou gerado.")
        return ""
